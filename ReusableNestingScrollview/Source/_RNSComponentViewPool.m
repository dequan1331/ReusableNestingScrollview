...
@interface _RNSComponentViewPool ()
...
@implementation _RNSComponentViewPool

+ (_RNSComponentViewPool *)shareInstance {
    static dispatch_once_t once;
    static _RNSComponentViewPool *componentPool = nil;
    dispatch_once(&once,^{
        componentPool = [[_RNSComponentViewPool alloc] init];
    });
...
- (__kindof UIView *)dequeueComponentViewWithIdentifier:(NSString*)identifier
    ...
    if(![viewClass isSubclassOfClass:[UIView class]]){
        NSAssert(NO, @"RNSComponentViewPool dequeue invalid view class:%@",viewClass);
    }
    
    __kindof UIView *dequeueView = [self _getViewOfClass:viewClass];
    [dequeueView setRNSId:identifier];
    return dequeueView;
}
...
-(void)_recycleView:(__kindof UIView *)view{
    ...
    if([[_dequeueViews allKeys] containsObject:classStr]){
        ...
        dispatch_semaphore_signal(_lock);
        NSAssert(NO, @"RNSComponentViewPool recycle invalid view");
    }
    ...
    if([[_enqueueViews allKeys] containsObject:classStr]){
        ...
        if(viewSet.count < RNSComponentViewPoolMaxResuableCount){
